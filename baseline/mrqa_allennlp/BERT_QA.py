...
class BERT_QA(Model):
    ...
    def forward(self,  # type: ignore
        ...
        output_dict: Dict[str, Any] = {"span_start_logits": span_start_logits,
        ...
        if span_start is not None and len(np.argwhere(span_start.squeeze(-1).squeeze(-1).cpu() >= 0)) > 0:
        ...
        for question_inds, question_instances_metadata in zip(per_question_inds, metadata):
            ...
            for i in range(len(question_inds)):
                ...
                output_dict["start_bias_weight"].append(util.masked_softmax(span_start_logits[best_span_ind],
                                                        repeated_passage_mask[best_span_ind])[best_span_cpu[best_span_ind][0]])
                output_dict["end_bias_weight"].append(util.masked_softmax(span_end_logits[best_span_ind],
                                                        repeated_passage_mask[best_span_ind])[best_span_cpu[best_span_ind][1]])
